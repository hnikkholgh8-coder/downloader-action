...
LOCK_FILE = os.path.join(APPDATA_DIR, "first_run.lock")
...
file_logger_handler = RotatingFileHandler(
    log_file_path,
    maxBytes=10 * 1024 * 1024,  # ۱۰ مگابایت حد نصاب حجم فایل
    backupCount=20,             # حفظ فایل‌های پشتیبان تا ۲۰ فایل
    encoding="utf-8"
)
...
DEFAULT_MODES = {
    ...
    "summary": {
        ...
        "model": "gemma3:1b",
        "prompt": (
            "SYSTEM MANDATE: You are a precise summarizing utility.\n"
            "STRICT LANGUAGE LOCK: You MUST summarize the input text in the EXACT SAME language as the input text itself. "
            "If the input is in Persian, your summary MUST be in Persian. "
            "If the input is in English, your summary MUST be in English. "
            "Do not translate or switch languages under any circumstances.\n"
            "STRICT RULES:\n"
            "- Condense the text, extracting only the critical arguments, essential details, and core context.\n"
...
class AppConfig:
    ...
    last_active_mode: str = "correction"
        ...
        try:
            ...
            if is_success:
                if self.current_mode == "spelling":
                    ...
                    # اصلاح راست‌چین سازی بلاک HTML به صورت صریح با ویژگی dir=rtl
                    styled_html = f'<div dir="rtl" style="white-space: pre-wrap; direction: rtl; text-align: right; font-family: Calibri, B Nazanin, Tahoma, sans-serif;">{diff_html}</div>'
...
class ModernAssistantApp(QObject):
    def __init__(self, q_app: QApplication):
        ...
        # بررسی اجرای دور اول برنامه به منظور نمایش پنجره راهنمای استفاده
        QTimer.singleShot(800, self.check_first_run_flow)

    def check_first_run_flow(self):
        try:
            if not os.path.exists(LOCK_FILE):
                self.show_first_run_tutorial()
                with open(LOCK_FILE, "w", encoding="utf-8") as f:
                    f.write(f"First run lock created at: {time.ctime()}")
        except Exception as e:
            logger.error(f"Error in first run flow: {e}")

    def show_first_run_tutorial(self):
        try:
            msg = QMessageBox()
            msg.setWindowTitle("راهنمای استفاده از دستیار هوشمند اکسیرپویان")
            msg.setText(
                "به دستیار هوشمند اکسیرپویان خوش آمدید!\n\n"
                "راهنمای گام‌به‌گام کارکرد نرم‌افزار:\n"
                "۱. ابتدا بخشی از یک متن را در هر محیطی (ورد، مرورگر وب، تلگرام و...) به حالت انتخاب (Highlight) درآورید.\n"
                f"۲. دکمه‌های میانبر [{self.config.hotkey_str.upper()}] را از روی کیبورد فشار دهید.\n"
                "۳. پنجره دستیار هوشمند باز شده و متن شما را بر اساس سربرگ انتخابی پردازش و اصلاح می‌کند.\n"
                "۴. با انتخاب دکمه 'اعمال و جایگذاری'، متن جدید به جای متن قبلی شما بازنشانی خواهد شد.\n\n"
                "این راهنما فقط در نخستین اجرای نرم‌افزار نمایش داده می‌شود."
            )
    ...
    def on_config_reload_triggered(self):
        try:
            ...
            self.start_hotkey_listener()
            # به روز رسانی متن راهنمای تولتیپ با کلید میانبر جدید
            if self.tray_icon:
                self.tray_icon.setToolTip(f"من سالم و سرحال در خدمت شما هستم (کلید میانبر: {self.config.hotkey_str.upper()})")
        except Exception as e:
    ...
    def setup_tray_icon(self):
        try:
            ...
            # ثبت متن سلامتی به همراه راهنمای میانبر سیستم به عنوان Tooltip آیکون ترِی
            self.tray_icon.setToolTip(f"من سالم و سرحال در خدمت شما هستم (کلید میانبر: {self.config.hotkey_str.upper()})")

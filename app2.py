...
class FloatingWindow(tk.Toplevel):
    def __init__(self, parent, captured_text: str, config: AppConfig, available_models: List[str], app_font: str):
        ...
        # انتخاب اولین حالت به عنوان حالت پیش‌فرض
    ...
    def setup_ui(self):
        ...
        # ساخت خودکار دکمه‌ها بر اساس فایل کانفیگ و مدیریت شکست خطوط (حداکثر ۴ ستون)
        MAX_COLUMNS = 4
        for index, (mode_id, mode_info) in enumerate(self.config.modes.items()):
            row = index // MAX_COLUMNS
            col = index % MAX_COLUMNS
            
            # جهت حفظ ظاهر راست به چپ، ستون‌ها معکوس نمایش داده می‌شوند
            col_rtl = MAX_COLUMNS - 1 - col
            label_text = mode_info.get("label", mode_id)
            ...
            btn = tk.Button(
                ...
                cursor="hand2", 
                font=(self.app_font, 8, "bold"), 
                command=lambda m=mode_id: set_mode(m)
            )
            btn.grid(row=row, column=col_rtl, sticky="nsew", padx=2, pady=2)
            tab_frame.grid_columnconfigure(col_rtl, weight=1)
            
            self.mode_buttons[mode_id] = btn

        # فعال‌سازی دیزاین کلید پیش‌فرض
        ...
        def on_paste_replace():
            try:
                ...
                if content and "در حال پردازش" not in content and "عذر می‌خوام" not in content:
                    ...
                    # ۳. شبیه‌سازی مطمئن Paste در یک Thread آسنکرون پس‌زمینه
                    def run_async_paste():
    ...
    def parent_api_call_bridge(self, text, model, mode, callback):
        ...
        # لود کردن مستقیم پرامپت اختصاصی مربوط به مود فعال از کانفیگ
        mode_info = self.config.modes.get(mode, {})
        
        # هندل کردن پرامپت هوشمند دوگانه فارسی/انگلیسی مخصوص بخش ترجمه
        if "prompt_to_fa" in mode_info and "prompt_to_en" in mode_info:
            ...
            # استفاده از پرامپت عمومی تک منظوره مربوط به این حالت
